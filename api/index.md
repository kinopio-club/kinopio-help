...
`POST`    | <code class="cards">/card/search</code>                 | Get all cards that match a query. Body object must contain `query`. Only matches cards created by the user. Searches are not case-insensitive                                       | `apiKey`
