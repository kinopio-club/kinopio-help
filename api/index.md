...
`POST`    | <code class="cards">/card/list</code>                   | Add card to a `listId` specified in request body. Body object must contain card `id`, and `listId`. If body object has `shouldPrepend: true`, the card will be added to the top of the list     | `canEditSpace`
...
`DELETE`  | <code class="cards">/card/list</code>                   | Remove card from the list that it's in. Body object must contain card `id`. The card's position will be shifted to the right of the list.                                       | `canEditSpace`
